...
def init_omni_lmm(model_path):
    ...
    if False:
        ...
        model = OmniLMMForCausalLM.from_pretrained(model_name, tune_clip=True, torch_dtype=torch.bfloat16).to(device='cuda', dtype=torch.bfloat16)
...
def expand_question_into_multimodal(question_text, image_token_len, im_st_token, im_ed_token, im_patch_token):
    if '<image>' in question_text[0]['content']:
        question_text[0]['content'] = question_text[0]['content'].replace('<image>', im_st_token + im_patch_token * image_token_len + im_ed_token)
    else:
        question_text[0]['content'] = im_st_token + im_patch_token * image_token_len + im_ed_token + '\n' + question_text[0]['content']
    return question_text

def wrap_question_for_omni_lmm(question, image_token_len, tokenizer):
    question = expand_question_into_multimodal(question, image_token_len, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_IMAGE_PATCH_TOKEN)
...
def process_conversation_file(file_path, chat_model):
    with open(file_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()

    conversations = []
    for i in range(0, len(lines), 3):  # 每三行一组（image, content, 空行）
        # 确保存在image和content对，并跳过空行
        if i + 1 < len(lines) and lines[i].strip() and lines[i+1].strip():
            image_path = lines[i].strip().split(':', 1)[1].strip()  # 修剪路径字符串的空格
            content = lines[i+1].strip().split(':', 1)[1]
            image_base64 = img2base64(image_path).decode('utf-8')
...
def save_results_to_file(results, file_path='result3_3.txt'):
    with open(file_path, 'w', encoding='utf-8') as f:
        for result in results:
            f.write(result + '\n')

if __name__ == '__main__':
    model_path = './MiniCPM-Llama3-V-2_5'
    chat_model = MiniCPMVChat(model_path)
    conversation_file = 'val1.txt'
    
    conversations = process_conversation_file(conversation_file, chat_model)

    results = []
    for i, input in enumerate(conversations, 1):  # 使用 enumerate 生成索引，从 1 开始计数
        answer = chat_model.chat(input)
        user_content = json.loads(input['question'])[-1]["content"]
        print(user_content)
        print("\n")
    ...
    # 统计生成的 answer 数量
    print(f"Total number of answers: {len(results)}")
